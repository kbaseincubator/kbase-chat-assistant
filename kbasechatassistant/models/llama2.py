...
class llama2(LanguageModel):
        def __init__(self, models_folder:Path, name: str = "llama-2-13b-chat-tok", chat_template: str = None, device:str='auto'):
        
            super().__init__(models_folder=models_folder, name = name, chat_template = chat_template)
            self.tokenizer = AutoTokenizer.from_pretrained(self.pretrained_model_name_or_path)
            self.model = LlamaForCausalLM.from_pretrained(self.pretrained_model_name_or_path, device_map = self.device)
            self.pipe = pipeline(
                model = self.model, 
